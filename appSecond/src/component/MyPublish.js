...
import {View,Text,Dimensions,StyleSheet,TouchableOpacity,ToastAndroid,ActivityIndicator,ScrollView} from 'react-native';
            ...
            <ScrollView>
                <View style={styles.box}>
                            ...
                            this.state.data.map((item,idx)=>{
                                return(
                                    <View key={idx} style={{height:40,flexDirection:'row',alignItems:'center',borderBottomWidth:0.5,borderBottomColor:'#a1a1a1'}}>
                                        <Text style={{paddingLeft:10,color:'#333',flex:0.6,fontSize:10}}>
                                             {item.title.length > 15 ? item.title.slice(0,15) + '...' : item.title}
                                        </Text>
                                        <Text style={{flex:0.35,color:'#333333',paddingLeft:20,fontSize:10}}>
                                            {item.create_at.slice(0,10)}
                                        </Text>
                                        <Text style={this.state.flag[idx] === '已回复' ? {color:'#f23030',paddingLeft:10,fontSize:10} :{color:'#333',paddingLeft:10,fontSize:10}}>
                    ...
                    <View style={[styles.bottomBtn,{flexDirection:'row',alignItems:'center'}]}>
                        <TouchableOpacity onPress={()=>this.deletePage()} style={[styles.touchBtn,styles.boxPosition,{marginLeft:30*screenScale}]}>
                            <Text style={{color:'#fff'}}>上一页</Text>
                        </TouchableOpacity>
                        <Text style={{marginLeft:100*screenScale}}>第 {this.state.page} 页</Text>
                        <TouchableOpacity onPress={()=>this.addPage()} style={[styles.touchBtn,styles.boxPosition,{marginLeft:90*screenScale}]}>
